...
8. `generate_static_newsletters.js` — **must run after** evidence packs / clean evidence /
   ...
   - `build_api_endpoints.js` — pre-render the classic `api.js` routes to static JSON.
...
- **`build_trends.js`** — Rolls the committed longitudinal record (`history/concept_evidence/*`, `issues/*/default.json`, `history/helio/*` + the current `helio_evidence.json`) into `publish/data/trends.json`: per-cycle research outcomes (confidence/decision/strength/findings), per-concept trajectories, the Helio UX-metric time series, and respondent quotes. Also folds in two **curated, committed** signal layers (data, not code; re-curated monthly — see OPERATIONS.md "Dashboard signals"): `netlify/content/voice_of_user.json` (the verbatim quotes that replace the harvested rotator pool → `quote_mode`) and `netlify/content/ux_signals.json` (the editorial read per Helio comparison → `ux_signals`, matched to comparisons by the renderer). The repo _is_ the database — versioned, diffable, auditable. Exports `buildTrends`/`loadUxSignals` for tests.
...
- **`render_trends_dashboard.js`** — Renders the trends dashboard, which **is the site homepage** (`publish/index.html`), from `trends.json`: shared theme + sidebar + hand-rolled SVG charts whose fills are CSS variables (so they follow light/dark). No chart lib, no React/Vite. Must be the **last** writer of `index.html` (see build order). The Helio section: titled comparisons (a real `comparison_title`) stay distinct and get a **succinct** title (`succinctTitle` drops "191 " number prefixes and collapses a trailing "…Page V1" → "v1"); concept-only comparisons **collapse to one block per concept** (the most informative kept); comparisons with **no real title and no concept are dropped** (the generic "Data Comparison" noise); each block renders **one colored bar per variant** (N-variant) with a legend; and a **localStorage-persisted multiselect** chooses which show. **Beneath each chart** (inside the card, so the filter hides it too) a **deterministic "Frontrunner" line** is computed from the same scores — the leading variant, its avg lift vs. baseline (`overall_score` excluded as a roll-up), metrics-led count, biggest mover, and the **regression** case (baseline still leads) — followed by the **signal**: the curated `ux_signals` read when one matches (by `compare_id` or title substring), else a computed fallback (`comparisonFrontrunner`/`matchUxSignal`/`frontrunnerBlock`). Concept labels come from the deck-link inference (`CONCEPT_HINTS` in `everpure_external_research_ingest.py`). Ends with **issue hero cards** (one per published month, from `trends.issues`) linking to each frozen issue.
- **`render_sitemap.js`** — Renders a branded, comprehensive sitemap (`publish/sitemap/index.html`) by **scanning the built `publish/` tree**, so it lists every page + API/data/history artifact that actually shipped and can't drift (mutually-exclusive buckets with a catch-all). Runs last.
- **`build_api_endpoints.js`** — Pre-renders the "classic" `api.js` routes (`/api/health`, `/api/metadata`, `/api/weeks`, `/api/findings`, `/api/summary`, `/api/decks`, `/api/deck-summary`, `/api/deck-details` + per-deck, `/api/deck-content`, `/api/static-summary`) to static JSON under `publish/api/` by invoking the real handler at build time — so the endpoints that only worked on the (removed) serverless host now work on static Pages. Filtered routes emit the unfiltered default payload.
...
- **`publish_issue_archives.js`** — Copies repo-tracked `issues/` + `history/` into `publish/`, generates the branded (via `dashboard_theme.js`) **issues index** (`publish/issues/index.html`) and a **separate activity-logs index** (`publish/activity/index.html`, the sidebar's "Activity log" target) + `publish/data/issues.json`. Each listing: card-style "Month · Issue NN" title with right-aligned `MD`/`JSON` chips and a "View issue"/"View log" button.
