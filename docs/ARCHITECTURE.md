...
- **`everpure_refresh.py`** — Refresh pipeline. Refreshes parsed JSON from a public Notion URL (`--source-url`) or local snapshot (`--html-path`), plus `newsletter_pack_90d.json` and `refresh_manifest.json`. `requests`-first with Playwright fallback (`--fetch-method playwright`). Records `source_fetch_method` / `source_fallback` markers.
