...
- **`render_trends_dashboard.js`** — Renders the trends dashboard, which **is the site homepage** (`publish/index.html`), from `trends.json`: shared theme + sidebar + hand-rolled SVG charts whose fills are CSS variables (so they follow light/dark). No chart lib, no React/Vite. Must be the **last** writer of `index.html` (see build order). The Helio section groups by `compare_id` (one block per compare page), titles by `comparison_title → concept → "Helio comparison"` (never a raw ULID; null concepts are backfilled by cross-referencing the same `test_id`), and has a **localStorage-persisted multiselect** to choose which comparisons show. Ends with **issue hero cards** (one per published month, from `trends.issues`) linking to each frozen issue.
