...
def cli() -> None:
    ap = argparse.ArgumentParser(
        description="Ingest exported deck PDFs into normalized text content"
    )
    ap.add_argument(
        "--data-dir",
    ...
    ap.add_argument(
        "--pdf-dir", required=True, help="Directory containing deck PDFs named <file_id>.pdf"
    )
    ap.add_argument(
        "--out-content", default=None, help="Optional explicit output path for deck_content.json"
    )
    ap.add_argument(
        "--out-summary",
        default=None,
        help="Optional explicit output path for deck_content_summary.json",
    )
    args = ap.parse_args()
    ...
    out_summary = (
        Path(args.out_summary) if args.out_summary else data_dir / "deck_content_summary.json"
    )
