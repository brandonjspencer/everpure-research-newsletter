...
const BOILERPLATE = [
  ...
  /intent/i,
  /effort/i,
  /supported by \d+ linked findings deck/i,
  /^captured in the /i,
  /a rundown of the testing/i,
  /executive brief/i,
  /pure storage executive brief/i,
  /everpure executive brief/i,
];
const STRONG = [
  /%/,
...
function collectLines(item) {
  ...
  [
    ...
    item.matched_summary_hint,
    item.matchedSummaryHint,
  ].forEach((v) => {
...
function collapseTitle(title) {
  ...
  t = t.replace(/\b(page|pages)\b/gi, "");
  t = t.replace(/\b(homepage|landing page|landing|pathfinder|internal)\b/gi, "$&");
...
function buildConcept(item) {
  ...
  const keyNums = uniq(
    collectLines(item).filter((l) =>
      /%|\bfrom\b.+\bto\b|increase|decrease|drop|boost|lift|decline|improv/i.test(l)
    )
  ).slice(0, 2);
  ...
  const nextStep =
    cleanLine(
      item.next_step_from_clean_evidence ||
        item.nextStepFromCleanEvidence ||
        item.rule_based_next_step ||
        item.ruleBasedNextStep ||
        item.next_step ||
        item.nextStep
    ) ||
  ...
  const confidence =
    cleanLine(
      item.rule_based_confidence ||
        item.ruleBasedConfidence ||
        item.confidence_level ||
        item.confidenceLevel
    ) || (proof && proof.score >= 7 ? "medium" : "low");
  const decision =
    cleanLine(
      item.rule_based_status || item.ruleBasedStatus || item.decision_status || item.decisionStatus
    ) || (proof && proof.score >= 7 ? "iterate" : "watch");
  const support = [];
  if (proof && proof.score >= 5) support.push(proof.line);
  if (weeks.length)
    support.push(
      `Seen in ${weeks.length} weekly update${weeks.length === 1 ? "" : "s"} (${weeks.join(", ")}).`
    );
  return {
    headline: title,
...
function mergeInMotion(items) {
  ...
  for (const item of items || []) {
    const titleKey = normalizeText(
      item.collapsed_headline || item.workstream || item.headline || item.title
    );
...
function renderMarkdown(news) {
  ...
  lines.push(
    `**Window:** ${news.window?.label || "30d"} (${news.window?.since || ""} to ${news.window?.until || ""})  `
  );
  ...
  (news.sections?.top_findings || []).forEach((item) => {
    ...
    lines.push(
      "**Confidence**  ",
      `${item.confidence_level || "low"} confidence • ${item.decision_status || "watch"}`,
      ""
    );
  });
  ...
  (news.sections?.in_progress || []).forEach((item) =>
    lines.push(
      `- **${item.workstream || item.headline || item.title}** — ${item.next_step || item.finding_statement || ""}`
    )
  );
...
function renderHtml(news) {
  const top = (news.sections?.top_findings || [])
    .map(
      (item) => `
    <section class="card">
    ...
    .join("\n");
  const comparisons = (news.sections?.comparison_tests || [])
    .map(
      (item) => `
    ...
    .join("\n");
  const inMotion = (news.sections?.in_progress || [])
    .map(
      (item) =>
        `<li><strong>${esc(item.workstream || item.headline || item.title)}</strong> — ${esc(item.next_step || item.finding_statement || "")}</li>`
    )
    .join("");
...
function main() {
  ...
  const strong = concepts
    ...
    .slice(0, 3);
  const watchRaw = concepts
    .filter((c) => c._score < 5)
    .sort((a, b) => (b.source_refs?.length || 0) - (a.source_refs?.length || 0))
    .slice(0, 8);
  const watch = mergeInMotion(watchRaw).slice(0, 5);
  const comparison = concepts
    .filter((c) =>
      /v1|v2|v3|r2|r3|baseline|comparison|variant/i.test(
        `${c.headline} ${c.finding_statement} ${c.evidence_snapshot}`
      )
    )
    .sort((a, b) => b._score - a._score)
    .slice(0, 4)
  ...
  const nextActions = dedupeActions(
    strong.map((x) => x.next_step).concat(comparison.map((x) => x.next_step))
  );
  ...
  current.sections.top_findings = strong.map((c) => ({
    headline: c.headline,
    finding_statement: c.finding_statement,
    proof_point: c.proof_point,
    ...
    JSON.stringify(
      {
        ...
        promoted_count: current.sections.top_findings.length,
