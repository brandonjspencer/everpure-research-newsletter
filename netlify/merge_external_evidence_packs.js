...
function titleTokens(title) {
  const stop = new Set([
    'the','and','for','with','from','this','that','page','pages','test','tests','testing','concept',
    'concepts','design','review','analysis','baseline','version','variant','variation','feedback','research',
    'content','current','signal','study','studies','round','rounds','user','users'
  ]);
  return uniq(normalize(title).toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
...
function conceptAliases(pack) {
  ...
  if (/homepage/.test(low) && /ai/.test(low)) aliases.push('homepage ai messaging', 'ai messaging', 'homepage ai');
  if (/pathfinder/.test(low) || /cta/.test(low)) aliases.push('pathfinder cta labels', 'cta labels', 'personalize', 'start here');
  if (/webinar/.test(low)) aliases.push('webinar registration', 'registration page', 'webinar');
  if (/book/.test(low) || /filter/.test(low)) aliases.push('this book', 'reader filter', 'book filter');
  if (/virtualization/.test(low)) aliases.push('virtualization campaign', 'virtualization', 'solutions page', 'campaign page');
  if (/summary/.test(low) && /ai/.test(low)) aliases.push('ai summary', 'ai summaries', 'book summary', 'results summary', 'publication summary');
  return uniq(aliases.map(a => normalize(a).toLowerCase()).filter(Boolean));
}

function evidenceConceptStrings(evidence) {
  const values = [];
  for (const key of ['concept', 'concept_title', 'concept_display', 'link_text', 'deck_title', 'source_title', 'title']) {
    if (evidence[key]) values.push(evidence[key]);
  }
  const inferred = evidence.inferred_concepts || evidence.concepts || evidence.matched_concepts || [];
  if (Array.isArray(inferred)) {
    for (const item of inferred) {
      if (typeof item === 'string') values.push(item);
      else if (item && typeof item === 'object') {
        values.push(item.title, item.name, item.label, item.concept_title, item.concept_display, item.concept_id, item.id);
      }
    }
  }
...
function matchScore(pack, evidence) {
  ...
  for (const alias of aliases) {
    if (!alias) continue;
    if (conceptStrings.some(c => c === alias || c.includes(alias) || alias.includes(c))) score += 20;
    if (alias.length >= 5 && text.includes(alias)) score += 10;
  }

  if (pack.concept_id && new RegExp(`\\b${String(pack.concept_id).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`).test(text)) score += 16;
  ...
  if (/pathfinder|cta/.test(packLow) && /\b(pathfinder|personalize|start here|cta|label)\b/.test(text)) score += 6;
  ...
  if (/virtualization/.test(packLow) && /\b(virtualization|campaign|solution|solutions)\b/.test(text)) score += 6;
...
function signalScore(text) {
  ...
  if (/\b(improved?|increase|decrease|lift|uplift|higher|lower|outperform|preferred|winner|winning|successful|success|engagement|sentiment|confidence|trust|clarity|comprehension|findability|discoverability|first-click|frequency)\b/i.test(t)) score += 5;
...
function evidenceRef(evidence, signals) {
  ...
  return {
    source_type: evidence.source_type || evidence.link_type || evidence.type || 'external_research_evidence',
...
function mergePack(pack, matches) {
  ...
  merged.external_evidence_refs = uniqObjects([...(merged.external_evidence_refs || []), ...refs]).slice(0, 12);
  merged.supporting_signals = uniq([...(merged.supporting_signals || []), ...cleanSignals]).slice(0, 18);
  merged.key_synthesis_signals = uniq([...(merged.key_synthesis_signals || []), ...cleanSignals]).slice(0, 12);
  merged.evidence_snapshot_rule_based = uniq([...(merged.evidence_snapshot_rule_based || []), ...cleanSignals]).slice(0, 10);
  merged.supporting_numbers = uniq([...(merged.supporting_numbers || []), ...externalNumbers]).slice(0, 18);
  merged.behavioral_signals = uniq([...(merged.behavioral_signals || []), ...extractBehavioralSignals(cleanSignals)]).slice(0, 18);
...
function extractBehavioralSignals(signals) {
  const terms = [];
  const rx = /\b(clarity|comprehension|sentiment|engagement|trust|confidence|findability|discoverability|conversion|success|frequency|preference|friction|effort|first-click)\b/ig;
...
function main() {
  ...
  const externalRaw = readJson(path.join(dataDir, 'external_research_evidence.json'), readJson(path.join(dataDir, 'external-research-evidence.json'), null));
  ...
  if (!evidenceItems.length) {
    report.warnings.push('No external evidence records found. Nothing was merged into evidence packs.');
  ...
  for (const fileName of packFiles) {
    ...
    if (changedCount > 0) {
      ...
      report.files_updated.push({ file: fileName, changed_pack_count: changedCount, total_pack_count: updatedPacks.length });
    }
  }
