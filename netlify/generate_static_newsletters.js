...
function buildList(items) {
  return items.map((item) => `<li><a href="${rel(item.href)}">${item.label}</a></li>`).join("\n");
}
...
(async () => {
  ...
  const newsletterLinks = {
    ...
    marketing: [
      {
        href: "/newsletter/marketing-activity-30d.html",
        label: "/newsletter/marketing-activity-30d.html",
      },
      {
        href: "/newsletter/marketing-activity-30d.md",
        label: "/newsletter/marketing-activity-30d.md",
      },
      {
        href: "/newsletter/marketing-activity-30d.json",
        label: "/newsletter/marketing-activity-30d.json",
      },
    ],
    discovery: [
  ...
  const apiLinks = [
    ...
    {
      href: "/api/newsletter-marketing-activity-30d.json",
      label: "/api/newsletter-marketing-activity-30d.json",
    },
    {
      href: "/api/newsletter-marketing-activity-30d.md",
      label: "/api/newsletter-marketing-activity-30d.md",
    },
  ];
  ...
  const dataLinks = [
    ...
    "/data/deck-details.json",
    "/data/deck_summary.json",
    "/data/deck-summary.json",
    "/data/deck_week_map.json",
    "/data/deck-week-map.json",
    ...
    .filter((href) => fs.existsSync(path.join(PUBLISH_DIR, href.replace(/^\//, ""))))
  ...
  fs.writeFileSync(
    path.join(NEWSLETTER_DIR, "default.html"),
    markdownToHtml(defaultJson.title || "Default monthly brief", defaultMd)
  );
  fs.writeFileSync(
    path.join(NEWSLETTER_DIR, "marketing-activity-30d.json"),
    JSON.stringify(marketingJson, null, 2)
  );
  fs.writeFileSync(path.join(NEWSLETTER_DIR, "marketing-activity-30d.md"), marketingMd);
  fs.writeFileSync(
    path.join(NEWSLETTER_DIR, "marketing-activity-30d.html"),
    markdownToHtml(marketingJson.title || "Marketing activity log", marketingMd)
  );

  fs.writeFileSync(
    path.join(NEWSLETTER_DIR, "index.html"),
    buildNewsIndex(statusJson, newsletterLinks)
  );
  fs.writeFileSync(
    path.join(PUBLISH_DIR, "index.html"),
    buildHomeHtml(statusOut, {
