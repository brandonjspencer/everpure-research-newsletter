...
function main() {
  ...
  const marketingIssue =
    readJson(path.join(newsletterDir, "marketing-activity-30d.json"), {}) || {};
  ...
  const manifest = {
    ...
    generated_at:
      defaultIssue.generated_at || marketingIssue.generated_at || new Date().toISOString(),
    ...
    source_refresh_manifest: exists(path.join(dataDir, "refresh_manifest.json"))
      ? "data/refresh_manifest.json"
      : null,
    history_paths: {
      weeks: exists(path.join(root, "history", "weeks", `${issueMonth}.json`))
        ? `history/weeks/${issueMonth}.json`
        : null,
      evidence_packs: exists(path.join(root, "history", "evidence_packs", `${issueMonth}.json`))
        ? `history/evidence_packs/${issueMonth}.json`
        : null,
      concept_evidence: exists(path.join(root, "history", "concept_evidence", `${issueMonth}.json`))
        ? `history/concept_evidence/${issueMonth}.json`
        : null,
