...
function countSheets(links) {
  return links.filter((link) => {
    const text = normalize(JSON.stringify(link)).toLowerCase();
    return (
      text.includes("docs.google.com/spreadsheets") ||
      text.includes("google_sheet") ||
...
function fetchCounts(fetchStatus) {
  ...
  for (const item of flat) {
    ...
    const failed =
      item.ok === false ||
      item.success === false ||
      /fail|error|denied|forbidden|unauth|missing/.test(status);
    const looksSheet =
      normalize(JSON.stringify(item)).toLowerCase().includes("sheet") ||
      normalize(JSON.stringify(item)).toLowerCase().includes("spreadsheet");
  ...
  return {
    external_sheet_success_count: Number(
      summary.sheet_success_count ||
        summary.google_sheet_success_count ||
        summary.success_count ||
        success ||
        0
    ),
    external_sheet_failure_count: Number(
      summary.sheet_failure_count ||
        summary.google_sheet_failure_count ||
...
function packExternalCount(raw) {
  ...
  return packs.filter(
    (pack) =>
      (pack.external_evidence_count || 0) > 0 || (pack.external_evidence_refs || []).length > 0
  ).length;
}
...
function injectArtifactLinks(filePath, hrefs) {
  ...
  const missing = hrefs.filter(
    (href) => !html.includes(href.replace(/^\//, "")) && !html.includes(href)
  );
...
function main() {
  ...
  const deckLinks = readJson(
    path.join(dataDir, "deck_links.json"),
    readJson(path.join(dataDir, "deck-links.json"), null)
  );
  const fetchStatus = readJson(
    path.join(dataDir, "deck_link_fetch_status.json"),
    readJson(path.join(dataDir, "deck-link-fetch-status.json"), null)
  );
  const externalEvidence = readJson(
    path.join(dataDir, "external_research_evidence.json"),
    readJson(path.join(dataDir, "external-research-evidence.json"), null)
  );
  const matchReport = readJson(
    path.join(dataDir, "external_research_evidence_match_report.json"),
    readJson(path.join(dataDir, "external-research-evidence-match-report.json"), null)
  );
  const defaultPacks = readJson(
    path.join(dataDir, "evidence_packs_default_30d.json"),
    readJson(path.join(dataDir, "evidence-packs-default-30d.json"), null)
  );
  ...
  const externalStatus = {
    ...
    deck_link_count: Number(
      deckLinks?.summary?.link_count || deckLinks?.summary?.total_links || linksArray.length || 0
    ),
    google_sheet_link_count: Number(
      deckLinks?.summary?.google_sheet_link_count || countSheets(linksArray)
    ),
    helio_link_count: Number(deckLinks?.summary?.helio_link_count || countHelio(linksArray)),
    external_evidence_count: Number(
      externalEvidence?.evidence_count ||
        externalEvidence?.summary?.evidence_count ||
  ...
  const outputs = {
    status: updateStatusFile(
      path.join(publishRoot, "status.json"),
      externalStatus,
      discoveryUpdates
    ),
    api_status: updateStatusFile(
      path.join(publishRoot, "api", "status.json"),
      externalStatus,
      discoveryUpdates
    ),
    ...
    newsletter_index: injectArtifactLinks(
      path.join(publishRoot, "newsletter", "index.html"),
      hrefs
    ),
  };
  ...
  console.log(
    JSON.stringify({ external_evidence: externalStatus, outputs, summary: summaryPath }, null, 2)
  );
}
