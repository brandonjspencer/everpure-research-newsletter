...
function buildPack(weeks, deckContentIndex, since, until) {
  ...
  const deckIds = [
    ...new Set((weeks || []).map((w) => (w.deck || {}).file_id).filter(Boolean)),
  ].sort();
...
function firstUsefulDeckSentence(deck) {
  const text = String((deck || {}).text_excerpt || (deck || {}).full_text || "")
    .replace(/\s+/g, " ")
    .trim();
  if (!text) return null;
  const parts = text
  ...
  for (const row of collectGroupRows(weeks, group)) {
    ...
    if (!counts.has(key))
      counts.set(key, {
        ...
        count: 0,
        latest_week_date: row.week_date,
      });
    const current = counts.get(key);
    current.count += 1;
    if ((row.week_date || "") > (current.latest_week_date || ""))
      current.latest_week_date = row.week_date;
  }
  return [...counts.values()]
    .sort(
      (a, b) =>
        b.count - a.count || (b.latest_week_date || "").localeCompare(a.latest_week_date || "")
    )
    .slice(0, limit);
}
...
function chooseWeeklyHighlights(weeks) {
  ...
  for (const week of weeks || []) {
    ...
    for (const group of priority) {
      const items = flattenItems(((week || {}).content_groups || {})[group] || []).filter(
        (item) => item.level <= 1 && item.label && !isFillerLabel(item.label)
      );
      for (const item of items) {
        picked.push({
          group,
...
function buildExecutiveSummary(windowedWeeks, pack, deckContentIndex) {
  const topId = (pack.top_identifiers || [])
    .slice(0, 3)
  ...
  const deckIds = [
    ...new Set(windowedWeeks.map((week) => (week.deck || {}).file_id).filter(Boolean)),
  ];
  const withDeckText = deckIds.filter((id) => deckContentIndex[id]);

  const parts = [];
  parts.push(
    `Across ${pack.overview.week_count} weekly updates, the strongest activity centered on ${Object.entries(
      pack.group_counts || {}
    )
      .sort((a, b) => b[1] - a[1])
      .slice(0, 2)
  ...
  if (topId.length)
    parts.push(
      `Recurring concept IDs included ${topId.join(", ")}, suggesting repeated iteration rather than one-off studies.`
    );
  if (topTesting.length)
    parts.push(`The most visible testing threads were ${topTesting.slice(0, 3).join("; ")}.`);
  if (topInProgress.length)
    parts.push(`In-progress work emphasized ${topInProgress.slice(0, 3).join("; ")}.`);
  if (withDeckText.length)
    parts.push(
      `${withDeckText.length} linked decks in this window currently have ingested text available for evidence-backed summarization.`
    );
  return parts.join(" ");
}
...
function formatImplication(audience, row) {
  if (audience === "exec")
    return `This theme recurred ${row.mentions} time(s), making it a strong candidate for leadership-level visibility and prioritization.`;
  if (audience === "ux")
    return `This theme appeared ${row.mentions} time(s), suggesting repeated validation or unresolved design questions worth further synthesis.`;
  if (audience === "product")
    return `This signal appeared ${row.mentions} time(s), which may indicate a decision, taxonomy, or flow issue worth follow-up.`;
...
function buildTopFindings(windowedWeeks, limit = 6) {
  const rows = extractFindings(windowedWeeks, null, null).filter(
    (row) => row.level <= 1 && row.label && !isFillerLabel(row.label)
  );
  const seen = new Set();
  const out = [];
  for (const row of rows) {
    ...
    seen.add(key);
    out.push({
      headline: row.identifier
        ? `${row.identifier} — ${sentenceCase(row.label)}`
...
function buildRecurringThemes(windowedWeeks, audience, limit = 6) {
  const rows = topGroupRows(windowedWeeks, "testing_concepts", 4)
    .map((row) => ({ ...row, type: "testing_concept" }))
  ...
  return rows.map((row) => ({
    ...
    source_refs: collectGroupRows(
      windowedWeeks,
      row.type === "testing_concept" ? "testing_concepts" : "findings"
    )
  ...
  for (const week of weekly) {
    ...
    out.push({
      title: primary.identifier
        ? `${primary.identifier} — ${sentenceCase(primary.label)}`
        : sentenceCase(primary.label),
      angle:
        audience === "exec"
...
function inferThemeBucket(label) {
  ...
  const buckets = [
    [
      "navigation_and_cta_clarity",
      ["navigation", "nav", "cta", "label", "menu", "pathfinder", "journey"],
    ],
    [
      "homepage_and_message_clarity",
      [
        "homepage",
        "headline",
        "messaging",
    ...
    [
      "knowledge_portal_and_platform_structure",
      ["knowledge portal", "platform", "taxonomy", "support", "flow", "search", "reader"],
    ],
    ...
    [
      "comparison_and_evaluation_behavior",
      ["comparison", "compare", "baseline", "competitive", "assessment", "review"],
    ],
...
function collectNonFillerRows(windowedWeeks, groups = GROUP_KEYS) {
  return extractFindings(windowedWeeks, null, null).filter(
    (row) => groups.includes(row.group) && row.level <= 1 && row.label && !isFillerLabel(row.label)
  );
}
...
function cleanNarrativeLabel(label) {
  ...
  text = text.replace(/\((?:in process|internal|received)\)/gi, "");
...
function comparisonLike(text) {
  const low = safeLower(text);
  return /(\bv\d+\b|\br\d+\b|\bvariation\b|\bvariations\b|\bbaseline\b|\bcomparison\b|\bcompare\b|\breview\b|\banalysis\b|\bcompetitive\b)/.test(
    low
  );
}
...
function outcomeLike(text) {
  const low = safeLower(text);
  return /(preferred|stronger|weaker|clearer|confusing|improved|improve|lift|increase|decrease|reduced|reduction|higher|lower|won|winner|performed|outperformed|underperformed|engagement|comprehension|sentiment|confidence|successful|found|showed|indicates|suggests)/.test(
    low
  );
}
  ...
  for (const week of windowedWeeks || []) {
    ...
    for (const item of items) {
      ...
      matches.push({
        ...
        deck_id: (week.deck || {}).file_id || null,
        label: targetLabel,
        child_labels: flattenItems(item.children || [])
          .map((child) => cleanNarrativeLabel(child.label || child.text || ""))
          .filter(Boolean),
      });
    }
  }
...
function explicitFindingNarrative(label, entries, deckContentIndex = {}) {
  ...
  const deckExcerpt = deckIds
    .map((id) => firstUsefulDeckSentence(deckContentIndex[id] || {}))
    .find((x) => x && outcomeLike(x));
...
function decisionStatusFor(group, label, count) {
  ...
  if (group === "findings") {
    if (/(baseline|variation|variations|review|analysis|feedback|summary)/.test(low))
      return "iterate";
...
function bucketFocusSentence(bucket) {
  const map = {
    ...
    homepage_and_message_clarity:
      "Homepage and messaging comprehension remain active decision areas.",
    knowledge_portal_and_platform_structure:
      "Information architecture and discoverability are still being worked through.",
    event_and_launch_experiences:
      "Event and launch experiences are producing concrete directional signal.",
    brand_and_rebrand_signals: "Brand and rebrand work is producing clearer directional feedback.",
    comparison_and_evaluation_behavior:
      "Comparative work is beginning to show which variants are stronger.",
...
function decisionMessage(decision) {
  const map = {
    ...
    iterate:
      "The signal is useful, but it points to another iteration rather than an immediate ship call.",
...
function collectComparisonCandidates(windowedWeeks, limit = 5, deckContentIndex = {}) {
  ...
  for (const week of windowedWeeks || []) {
    for (const group of ["findings", "testing_concepts"]) {
      ...
      for (const item of items) {
        ...
        const childLabels = flattenItems(item.children || []).map((child) =>
          cleanNarrativeLabel(child.label || child.text || "")
        );
  ...
  for (const row of rows) {
    ...
    current.mentions += 1;
    current.child_labels = [
      ...new Set([...(current.child_labels || []), ...(row.child_labels || [])]),
    ];
    if (current.source_refs.length < 3)
      current.source_refs.push(traceRef(row.week_date, row.deck_id, row.record_id));
  ...
  return [...grouped.values()]
    ...
    .map((row) => {
      const entries = rows
        .filter((r) => r.group === row.group && r.label === row.label)
        .map((r) => ({ week_date: r.week_date, deck_id: r.deck_id, child_labels: r.child_labels }));
...
function buildStrategicThemeClusters(windowedWeeks, audience, limit = 5) {
  ...
  for (const row of rows) {
    ...
    if (!current.examples.includes(sentenceCase(row.label)) && current.examples.length < 4)
      current.examples.push(sentenceCase(row.label));
    if (current.refs.length < 4)
      current.refs.push(traceRef(row.week_date, (row.deck || {}).file_id || null, row.record_id));
  }
...
function formatClusterImplication(audience, cluster) {
  ...
  if (audience === "marketing")
    return `${base} This is useful for showing how research focus is clustering across the month.`;
  if (audience === "product")
    return `${base} This is likely to influence a decision, labeling choice, or flow change.`;
...
function extractMetricSnippetsFromText(text, limit = 2) {
  ...
  return parts
    .filter(
      (part) =>
        /(\d+%|\+\d+%|-\d+%|\b\d+\b)/.test(part) &&
        /(increase|decrease|lift|drop|engagement|comprehension|sentiment|confidence|successful|participants|preferred|improved|reduced|higher|lower)/i.test(
          part
        )
    )
    .slice(0, limit)
...
function evidenceSnapshot(entries, deckContentIndex = {}) {
  const weekDates = uniqueValues(entries.map((e) => e.week_date))
    .sort()
    .reverse();
  ...
  if (weekDates.length > 1) {
    supportingSignals.push(
      `Repeated across ${weekDates.length} weekly updates (${weekDates.slice(0, 3).join(", ")}${weekDates.length > 3 ? ", …" : ""})`
    );
  ...
  if (childLabels.length) {
    supportingSignals.push(
      `Named variants or affected areas: ${childLabels.slice(0, 5).join(", ")}`
    );
  }
  if (deckIds.length) {
    supportingSignals.push(
      `Supported by ${deckIds.length} linked findings deck${deckIds.length === 1 ? "" : "s"}`
    );
  }
...
function buildValidatedFindings(windowedWeeks, audience, limit = 5, deckContentIndex = {}) {
  ...
  for (const row of ranked) {
    ...
    const evidence = evidenceSnapshot(entries, deckContentIndex);
    const findingStatement = insightSentenceForConcept(
      row.label,
      entries,
      "findings",
      deckContentIndex
    ...
    const confidence_level = confidenceLevelFor(
      "findings",
      plain,
      row.count,
      Boolean(evidence.metric_snippets.length || evidence.deck_count)
    );
    out.push({
      headline: plain,
...
function buildEmergingSignals(windowedWeeks, audience, limit = 5, deckContentIndex = {}) {
  return topGroupRows(windowedWeeks, "testing_concepts", limit).map((row) => {
    ...
    return {
      signal: plain,
      finding_statement:
        insightSentenceForConcept(row.label, entries, "testing_concepts", deckContentIndex) ||
        `This concept is active in the current window and is useful for narrowing the next iteration.`,
      evidence_snapshot: evidence.summary,
      supporting_signals: evidence.supporting_signals,
      key_numbers: evidence.metric_snippets,
...
function buildWorkstreamsToWatch(windowedWeeks, audience, limit = 5, deckContentIndex = {}) {
  return topGroupRows(windowedWeeks, "in_process", limit).map((row) => {
    ...
    return {
      workstream: plain,
      finding_statement:
        insightSentenceForConcept(row.label, entries, "in_process", deckContentIndex) ||
        `${plain} is still active and should stay visible as a tracked workstream.`,
      evidence_snapshot: evidence.summary,
      supporting_signals: evidence.supporting_signals,
      key_numbers: evidence.metric_snippets,
...
function buildVolumeSnapshot(windowedWeeks) {
  ...
  return {
    ...
    average_items_per_week: perWeek.length
      ? Number((totals.total_items / perWeek.length).toFixed(1))
      : 0,
    totals,
    weeks: perWeek,
  };
...
function buildActivityLogEntries(windowedWeeks, limit = 6) {
  return chooseWeeklyHighlights(windowedWeeks)
    .slice(0, limit)
    .map((week) => ({
      week_date: week.week_date,
...
function buildAudienceSummary(
  ...
  if (comparisonTests[0])
    parts.push(
      `The clearest comparison thread this month is ${comparisonTests[0].test_name.toLowerCase()}, and the best next move is to use it to narrow the next round rather than treat it as broadly ship-ready.`
    );
  if (!shipRecommendations.length)
    parts.push(
      "Taken together, the current 30-day window supports another focused iteration cycle more than a broad shipping decision."
    );
...
function buildEditorialRecommendations(
  ...
  recommendations.push(
    "Only treat an item as a finding when the issue can say in plain English what the research actually surfaced."
  );
  recommendations.push(
    "Pair every called-out concept with a next step so the issue reads like a decision brief, not a status report."
  );
  if (deckBackedInsights.length)
    recommendations.push(
      "Use quantitative or outcome language from the deck excerpts wherever possible to validate the claims in the narrative."
    );
  return recommendations;
}
...
function buildNewsletter(windowedWeeks, deckContentIndex, bounds, options = {}) {
  ...
  const deckBackedInsights = buildDeckBackedInsights(
    windowedWeeks,
    deckContentIndex,
    tone === "brief" ? 3 : 6
  );
  const validatedFindings = buildValidatedFindings(
    windowedWeeks,
    audience,
    tone === "brief" ? 3 : 5,
    deckContentIndex
  );
  const comparisonTests = collectComparisonCandidates(
    windowedWeeks,
    tone === "brief" ? 3 : 5,
    deckContentIndex
  );
  const emergingSignals = buildEmergingSignals(windowedWeeks, audience, tone === "brief" ? 3 : 5);
  const workstreamsToWatch = buildWorkstreamsToWatch(
    windowedWeeks,
    audience,
    tone === "brief" ? 3 : 5
  );
  const strategicThemes = buildStrategicThemeClusters(
    windowedWeeks,
    audience,
    tone === "brief" ? 3 : 5
  );
  const shipRecommendations = validatedFindings.filter((row) => row.decision_status === "ship");
  const iterateRecommendations = validatedFindings.filter(
    (row) => row.decision_status === "iterate"
  );
  const watchItems = [...workstreamsToWatch.slice(0, 3), ...emergingSignals.slice(0, 2)].slice(
    0,
    tone === "brief" ? 3 : 5
  );
  ...
  const issueHighlights = buildIssueHighlights(
    validatedFindings,
    strategicThemes,
    workstreamsToWatch,
    audience,
  ...
  const newsletter = {
    ...
    window: {
      since: bounds.since,
      until: bounds.until,
      label: bounds.window_label,
      days: bounds.days,
    },
    overview: pack.overview,
    executive_summary: buildAudienceSummary(
      audience,
      tone,
      mode,
      windowedWeeks,
    ...
    sections: {
      ...
      active_workstreams: workstreamsToWatch.map((row) => ({
...
function renderNewsletterMarkdown(newsletter) {
  ...
  lines.push(
    `_Audience: ${newsletter.audience} | Tone: ${newsletter.tone} | Mode: ${newsletter.mode}_`
  );
  lines.push("");
  ...
  if (newsletter.mode === "activity_log") {
    ...
    for (const row of newsletter.sections.validated_findings || []) {
      ...
      if ((row.supporting_signals || []).length)
        lines.push(`- **Supporting signals:** ${(row.supporting_signals || []).join(" | ")}`);
      if ((row.key_numbers || []).length)
        lines.push(`- **Key synthesis signals:** ${(row.key_numbers || []).join(" | ")}`);
      lines.push(`- **What we should do next:** ${row.next_step}`);
      lines.push(
        `- **Decision / confidence:** ${sentenceCase(row.decision_status)} / ${confidenceMessage(row.confidence_level)}`
      );
      lines.push("");
    }
...
exports.handler = async (event, context) => {
  ...
  const deckContentIndex = Object.fromEntries(
    (deckContent || []).filter((d) => d && d.file_id).map((d) => [d.file_id, d])
  );
  ...
  if (route === "deck-summary")
    return ok({ deck_summary: deckSummary, deck_content_summary: deckContentSummary }, freshness);
  ...
  if (route === "summary") {
    const filteredWeeks = filterWeeks(weeks, params);
    return ok(
      buildPack(filteredWeeks, deckContentIndex, params.since || null, params.until || null),
      freshness
    );
  }
  ...
  if (newsletterAliases[route]) {
    ...
    const filteredWeeks = filterWeeks(weeks, {
      ...mergedParams,
      since: bounds.since,
      until: bounds.until,
    });
    ...
    if (
      route.endsWith(".md") ||
      safeLower(params.format) === "markdown" ||
      safeLower(params.output) === "markdown"
    ) {
      return okText(
