...
function mergeKnowledgePortalInHtml(html) {
  ...
  const kpPattern =
    /### Knowledge Portal Landing Page[\s\S]*?- \*\*Decision \/ confidence:\*\*[^\n]*\n### Knowledge Portal Homepage[\s\S]*?- \*\*Decision \/ confidence:\*\*[^\n]*/m;
...
function normalizeJson(jsonText) {
  ...
  for (const bucket of buckets) {
    ...
    for (const item of arr) {
      ...
      if (
        title.includes("knowledge portal landing page") ||
        title.includes("knowledge portal homepage")
      ) {
        if (!merged) {
          out.push({
            title: "Knowledge Portal structure",
            finding_statement:
              "Knowledge portal and platform work is converging on naming and structure, which makes discoverability the main problem to solve next.",
            evidence_snapshot:
              "Captured in the 2026-03-26 weekly update; Supported by 1 linked findings deck",
            supporting_signals: [
              "Captured in the 2026-03-26 weekly update",
              "Supported by 1 linked findings deck",
            ],
            next_step:
              "Simplify structure and naming, then validate whether people understand the destination and can find the right entry point faster.",
