...
class EverpureStore:
    def __init__(self, data_dir: str):
        ...
        self.deck_content_summary = maybe_load_json(
            os.path.join(data_dir, "deck_content_summary.json"), {}
        )
...
def flatten_items(
    items: Iterable[Dict[str, Any]], parent: Optional[str] = None
) -> List[Dict[str, Any]]:
    out = []
    for item in items:
        text = item.get("text", "")
...
def extract_findings(
    weeks: List[Dict[str, Any]], group: Optional[str] = None, query: Optional[str] = None
) -> List[Dict[str, Any]]:
    rows = []
...
def build_newsletter_pack(
    store: EverpureStore, since: Optional[str] = None, until: Optional[str] = None
) -> Dict[str, Any]:
        ...
        timeline.append(
            {
                ...
                "counts": {
                    g: len(flatten_items(w["content_groups"].get(g, []))) for g in GROUP_KEYS
                },
        ...
        recent_weeks.append(
            {
                ...
                "highlights": {
                    ...
                    for g in GROUP_KEYS
                    if flatten_items(w["content_groups"].get(g, []))
                },
            }
        )
    ...
    return {
        ...
        "top_labels": [
            {"label": label, "count": count} for label, count in by_label.most_common(25)
        ],
        "timeline": timeline,
...
def write_newsletter_pack(
    data_dir: str, since: Optional[str], until: Optional[str], out_path: str
) -> Dict[str, Any]:
...
def create_app(data_dir: str):
    ...
    @app.get("/summary")
    def summary():
        pack = build_newsletter_pack(
            store, since=request.args.get("since"), until=request.args.get("until")
        )
        return jsonify(pack)
