...
def resolve_access_token(args: argparse.Namespace) -> str:
    ...
    if args.client_id and args.client_secret and args.refresh_token:
        return issue_refresh_token_access_token(
...
def fetch_and_save(
    ...
    refreshed = build_deck_details(
        weeks=weeks,
        decks=[{"file_id": d["file_id"], "url": d.get("canonical_url")} for d in deck_details],
...
def cli() -> None:
    ap = argparse.ArgumentParser(
        description="Fetch Google Slides deck artifacts using OAuth access token, refresh token, or service account"
    )
    ap.add_argument(
        "--data-dir",
    ...
    ap.add_argument(
        "--file-id",
        action="append",
        help="Specific Google file ID to fetch. Repeat flag to fetch multiple.",
    )
    ap.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Limit number of deck IDs when fetching from deck_details.json",
    )
