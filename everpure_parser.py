...
def parse_html(html: str) -> Dict[str, Any]:
    ...
    for block in blocks:
        ...
        if btype == "text" and links:
            for link in links:
                ...
                if slide and current_week["deck"] is None:
                    ...
                    if "notion.site" in host or "notion.so" in host:
                        current_week["linked_pages"].append(
                            {"title": link["label"], "url": link["url"]}
                        )
                    else:
                        current_week["linked_files"].append(
        ...
        if btype == "toggle" and text:
            current_week["toggles"].append(
                {"title": text, "expanded": False, "children_present": False}
            )
            continue
        ...
        if btype == "text" and text:
            ...
            if mapped in current_week["content_groups"]:
                ...
                only_link_text = (
                    bool(links)
                    and normalize_space(" ".join(l["label"] for l in links))
                    == text.replace("🧠 ", "").replace("👉 ", "").strip()
                )
                if current_group:
                    current_week["content_groups"][current_group].append(
                        {"text": text, "level": 0, "children": []}
                    )
                elif not only_link_text:
                    current_week["content_groups"]["other"].append(
                        {"text": text, "level": 0, "children": []}
                    )
            continue
...
def main():
    ap = argparse.ArgumentParser(
        description="Parse Everpure Notion HTML export into normalized JSON."
    )
    ...
    if args.output_dir:
        ...
        manifest = {
            ...
            "outputs": {
                k: os.path.join(os.path.abspath(args.output_dir), f"{k}.json")
                for k in ("metadata", "weeks", "decks", "summary")
            },
        }
